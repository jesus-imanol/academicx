...
const SubjectDetailModal = ({ isOpen, onClose, asignatura, programa }) => {
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-primary/20">
              <Icon name="import_contacts" className="text-primary text-2xl" />
            </div>
            <div>
              <h3 className="text-white text-xl font-bold">{asignatura.nombre}</h3>
              <p className="text-[#a19cba] text-sm">Asignatura</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#1d1b27] rounded-lg p-4 border border-white/5">
              ...
              <p className="text-white text-sm font-mono break-all">{asignatura.id}</p>
            </div>
            
            <div className="bg-[#1d1b27] rounded-lg p-4 border border-white/5">
              <p className="text-[#a19cba] text-xs font-medium mb-1">Nombre de la Asignatura</p>
              <p className="text-white text-sm font-bold">{asignatura.nombre}</p>
            </div>
            
            <div className="bg-[#1d1b27] rounded-lg p-4 border border-white/5">
              <p className="text-[#a19cba] text-xs font-medium mb-1">Cuatrimestre</p>
              <div className="flex items-center gap-2">
                <Icon name="schedule" className="text-cyan-500 text-2xl" />
                <p className="text-white text-2xl font-bold">{asignatura.cuatrimestre}°</p>
              </div>
            </div>
            ...
            <div className="bg-[#1d1b27] rounded-lg p-4 border border-white/5">
              ...
              <p className="text-white text-sm">
                {new Date(asignatura.createdAt).toLocaleDateString('es-MX', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                })}
          ...
          {programa && (
            <div className="rounded-lg p-4 border border-primary/20" style={{background: 'linear-gradient(to right, rgb(var(--primary) / 0.1), rgb(6 182 212 / 0.1))'}}>
              <div className="flex items-start gap-3">
                <Icon name="school" className="text-primary text-3xl mt-1" />
                <div className="flex-1">
                  <p className="text-white font-bold text-lg mb-1">
                    {programa.nombre}
                  </p>
                  <p className="text-[#a19cba] text-sm">
                    Programa de Estudio • {programa.cantidadCuatrimestres} Cuatrimestres
                  </p>
                  <div className="mt-2 flex items-center gap-2">
                    <Icon name="info" className="text-cyan-500 text-sm" />
                    <p className="text-[#a19cba] text-xs">
                      ID: {asignatura.programaEstudioId}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-[#1d1b27] rounded-lg p-4 border border-white/5">
            ...
            <p className="text-white text-sm">
              {new Date(asignatura.updatedAt).toLocaleString('es-MX', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
